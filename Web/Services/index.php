...
$app->hook('slim.before.dispatch', function () use ($app, $server, $registry) {
    ...
    if ($registry->IsSecure($routeName)) {
        ...
        $userRepository = new UserSessionRepository();
        $session = $userRepository->LoadBySessionToken($sessionToken);
        ...
        if (!$registry->IsUserAllowedApiAccess(routeName: $routeName, userId: $session->UserId)) {
            $app->halt(
